<?php
   ...
   $proedrostmhmatos = $_POST['proedrostmhmatos'];
   ...
   if(!strlen($username) or !strlen($password) or !strlen($idruma) or !strlen($sxolh) or !strlen($email) or !strlen($thlefwno) or !strlen($proedrostmhmatos)) {
   ...
   $stmt = $conn->prepare("INSERT INTO grammateies (username,password,idruma,sxolh,email,thlefwno,proedrostmhmatos) VALUES(?,?,?,?,?,?,?)");
   $stmt->bind_param("sssssss",$username,$password,$idruma,$sxolh,$email,$thlefwno,$proedrostmhmatos);
   $stmt->execute();
